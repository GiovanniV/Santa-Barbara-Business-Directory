...
<div id="trainingrecords-attachments" target=".trainingrecords-attachment-list">
	...
	<form action="<?php echo site_url('admin/business/uploadsearchbgfile');?>" method="post" enctype="multipart/form-data">
	    <ol class="filelist">
	    </ol>	
	...
	jQuery('#'+tab+' > form').ajaxForm({
		...
		complete: function(xhr) {
			...
			if(response.error==0)
			{
				...
				window.parent.jQuery('#search_bg').attr("value",response.name);
				window.parent.jQuery('#search_bg').trigger('change');
			}
			else
			{
				...
				window.parent.jQuery('#search_bg-error').html(error);
			}
